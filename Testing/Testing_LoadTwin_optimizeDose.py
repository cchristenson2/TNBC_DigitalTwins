...
if __name__ == '__main__':
    ...
    start = time.time()
    output = twin.optimize_doseMin(problem, problem_args, method = 2)
    print('COBYLA optimization time = ' + str(time.time() - start))
    ...
    # start = time.time()
    # output = twin.optimize_doseMin(problem, problem_args, method = 2)
    # print('SLSQP optimization time = ' + str(time.time() - start))
    # optimal_simulation = twin.predict(treatment = output[1], threshold = 0.25, plot = True)
