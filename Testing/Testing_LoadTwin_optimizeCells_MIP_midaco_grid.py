...
if __name__ == '__main__':
    ...
    for i, delivs in enumerate(delivs_vec):
        for j, tol in enumerate(tol_vec):
    ...
    temp = soc_simulation['cell_tc']
    plt.plot(t, np.mean(temp, axis = 1), color = 'black', label = 'SoC')
    tol = 0.0
    for i, delivs in enumerate(delivs_vec):
        if i < 5:
            temp = np.array(results[str(delivs)+'_'+str(tol)]['simulation']['cell_tc'])
            plt.plot(t, np.mean(temp, axis = 1), label = 'Deliveries: '+str(delivs))
    plt.legend()  
    plt.title('Tolerance = '+str(tol*100)+'%')
