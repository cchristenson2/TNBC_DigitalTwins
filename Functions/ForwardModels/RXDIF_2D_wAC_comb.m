%{ 
Forward model for the reaction diffusion equation with AC treatment + Paclitaxel in 2D
...
Input:
    ...
    - params: Struct containing all current parameter values (if valid)
    - dt: time step
...
function [N_sim, drug_TC] = RXDIF_2D_wAC_comb(tumor, params, dt, varargin)
    ...
    for k = 2:nt        
        ...
        for y = 1:sy
            for x = 1:sx
                boundary = tumor.bcs(y,x,:);
                
                %FDM in Y direction
                if(boundary(1)==0)
                    inv_y = params.d*(N(y+1,x,k-1)-2*N(y,x,k-1)+N(y-1,x,k-1))/(tumor.h^2);

                elseif(boundary(1)==1)
                    inv_y = params.d*(-2*N(y,x,k-1)+2*N(y-1,x,k-1))/(tumor.h^2);
                ...
                %FDM in X direction
                if(boundary(2)==0)
                    inv_x = params.d*(N(y,x+1,k-1)-2*N(y,x,k-1)+N(y,x-1,k-1))/(tumor.h^2);

                elseif(boundary(2)==1)
                    inv_x = params.d*(-2*N(y,x,k-1)+2*N(y,x-1,k-1))/(tumor.h^2);
                ...
                prolif   = N(y,x,k-1)*params.k(y,x,k-1)*(1-N(y,x,k-1));
                ...
                if(~isempty(delivs))
                    ...
                    treat = params.alpha*(drug1+drug2)*tumor.AUC(y,x)*N(y,x,k-1);
                end
                drug_TC(1,k) = drug1;
                drug_TC(2,k) = drug2;
                ...
                if(pac_exists==1)
                    ...
                    if(~isempty(delivs_pac))
                        ...
                        treat = treat + params.alpha_pac*drug3*tumor.AUC(y,x)*N(y,x,k-1);
                    end
                    drug_TC(3,k) = drug3;
                end

                N(y,x,k) = N(y,x,k-1) + dt*(invasion + prolif - treat);
            end
        end
    end
